...
// constexpr bool index() const noexcept;
...
// Test noexcept
template <class T>
concept IndexNoexcept =
    requires(T t) {
      { t.index() } noexcept;
    };

struct Foo {};
static_assert(!IndexNoexcept<Foo>);

static_assert(IndexNoexcept<std::expected<int, int>>);
static_assert(IndexNoexcept<const std::expected<int, int>>);
