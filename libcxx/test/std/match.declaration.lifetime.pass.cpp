...
bool unnamed_value_pattern_does_not_initialize() {
  int alive = 0;
  int copies = 0;
  UnnamedTracked subject(&alive, &copies);
  int observed = subject match {
    case UnnamedTracked if (alive == 1) => alive;
    case _ => -1;
  };
...
bool failed_guard_does_not_initialize_unnamed_values() {
  int alive = 0;
  int copies = 0;
  UnnamedTracked subject(&alive, &copies);
  int observed = subject match {
...
int main(int, char**) {
  ...
  if (!indirect_subject_lives_through_controlled_statement(2))
    return 6;
  if (!unnamed_value_pattern_does_not_initialize())
    return 7;
  if (!unnamed_reference_pattern_does_not_copy())
    return 8;
  if (!failed_guard_does_not_initialize_unnamed_values())
    return 9;
  return 0;
}
