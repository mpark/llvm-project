...
#pragma clang __debug dump match (x) { case _ if (T guard = value; true) => 0; case _ => 1; }
  ...
  // expected-warning@+1 {{value-dependent expression}}
#pragma clang __debug dump match (x) { case _ if (T guard = value) => 0; case _ => 1; }

  // expected-warning@+1 {{value-dependent expression}}
#pragma clang __debug dump match (x) { case sizeof(T) => 0; case _ => 1; }

  // expected-warning@+1 {{value-dependent expression}}
#pragma clang __debug dump match (x) -> unsigned { case _ => sizeof(T); }

  // expected-warning@+1 {{type-dependent expression}}
#pragma clang __debug dump match (x) -> T { case _ => value; }
}

template <class T>
T test_statement_handler_dependence(int x, T value) {
  // expected-warning@+1 {{value-dependent expression}}
#pragma clang __debug dump match (x) { case _ => return value; }
}
