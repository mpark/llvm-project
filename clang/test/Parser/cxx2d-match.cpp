// RUN: %clang_cc1 -std=c++2d -fsyntax-only -fpattern-matching -fcxx-exceptions -Wno-unused-variable -Wno-unused-value -verify %s
...
void test_binding_pattern(int i) {
  ...
  i match { let x => 0; };
  i match { let x => x; };
  i match { let [x] => 0; }; // expected-error {{cannot bind non-class, non-array type 'int'}}
  int i1[1] = {0};
  i1 match { let [x] => 0; };
  i1 match { let [x] => x; };
  int i2[2] = {0, 0};
