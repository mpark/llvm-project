...
template<>
struct std::alternative_traits<CVQualifiedChoice> {
  static constexpr alternative_info alternatives[] = {
    ^^int, ^^const int
  };
  static constexpr bool has_residual_states = false;

  static constexpr __SIZE_TYPE__ index(const CVQualifiedChoice& choice) noexcept {
    return choice.state;
  }
  ...
  static constexpr decltype(auto) get(Self&& choice) {
    if constexpr (I == 0)
      ...
      return (static_cast<Self&&>(choice).constant);
  }
};
...
int cv_qualified_type_selectors(CVQualifiedChoice choice) {
  return match (choice) {
    case { int: auto value } => value;
    case { const int: auto value } => value;
  };
}
