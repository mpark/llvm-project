...
#define RECORD(X) EmitRecordID(X, #X, Stream, Record)
  ...
  RECORD(STMT_SWITCH);
  RECORD(EXPR_INSPECT);
  RECORD(STMT_WILDCARDPATTERN);
  RECORD(STMT_IDENTIFIERPATTERN);
  RECORD(STMT_EXPRESSIONPATTERN);
  RECORD(STMT_WHILE);
  RECORD(STMT_DO);
  RECORD(STMT_FOR);
  RECORD(STMT_GOTO);
  ...
  RECORD(EXPR_LAMBDA);
  RECORD(EXPR_MATCH_SELECT);
  RECORD(PATTERN_WILDCARD);
  RECORD(PATTERN_OPTIONAL);
#undef RECORD
}
