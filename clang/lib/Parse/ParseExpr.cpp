...
ExprResult Parser::ParseRHSOfBinaryExpression(ExprResult LHS,
  ...
  while (true) {
    ...
    // Remember the precedence of this operator. ParseRHSExprOfBinaryExpression
    // may update NextTokPrec to the precedence of the operator immediately to
...
ExprResult Parser::ParseRHSExprOfBinaryExpression(ExprResult& LHS,
  ...
  if (RHS.isInvalid()) {
    ...
    Actions.CorrectDelayedTyposInExpr(LHS);
    if (TernaryMiddle && TernaryMiddle->isUsable())
      *TernaryMiddle = Actions.CorrectDelayedTyposInExpr(*TernaryMiddle);
    LHS = ExprError();
  }

  NextTokPrec =
      getBinOpPrecedence(Tok, GreaterThanIsOperator, getLangOpts().CPlusPlus11,
                         getLangOpts().PatternMatching);
  ...
  if (ThisPrec < NextTokPrec || (ThisPrec == NextTokPrec && isRightAssoc)) {
    if (!RHS.isInvalid() && RHSIsInitList) {
      Diag(Tok, diag::err_init_list_bin_op) << /*LHS*/ 0 << PP.getSpelling(Tok)
                                            << Actions.getExprRange(RHS.get());
      RHS = ExprError();
    }
