...
bool Type::isConstevalOnly() const {
  ...
  if (TypeBits.ConstevalOnly)
    return true;
  // Pointers and references to consteval-only types are themselves
  // consteval-only. Check the pointee dynamically rather than relying solely on
  // the bit captured at construction time, which may have been computed before
  // the pointee record's consteval-only-ness was known.
...
bool MemberPointerType::isSugared() const {
  // A splice qualifier ([:R:]::*) is always sugar for its resolved
  // (canonical) member-pointer type.
...
DeducedType::DeducedType(TypeClass TC, DeducedKind DK,
  ...
  case DeducedKind::Deduced:
    ...
    setConstevalOnly(DeducedAsTypeOrCanon->isConstevalOnly());
    break;
